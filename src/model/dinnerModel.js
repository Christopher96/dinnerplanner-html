...
class DinnerModel extends Observable {
    ...
    getDishById(id) {
        return this.getFullMenu().find(dish => dish.id === id);
    }
    ...
    getDishPrice(dish) {
        // return dish.ingredients.reduce((acc, ing) => acc + ing.price, 0);
    ...
    addDishToMenu(id) {
        this.getDish(id)
            .then(function(dish) {
                let found = this.getDishById(id);
                if(found) {
                    this.removeDishFromMenu(found.id);
                }
                found = this.getSelectedDish(dish.dishTypes[0]);
                if(found) {
                    this.removeDishFromMenu(found.id);
                }
                this.menu.push(dish);
    ...
    getConfig() {
        return fetch('src/config.json', { mode: 'no-cors' })
