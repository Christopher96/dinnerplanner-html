...
describe("DinnerPlanner App", () => {
    ...
    describe("Single Dish View", () => {
        beforeEach(() => {
            ...
            new SingleDishView(document.getElementById("page-content"), model, dish).render(true);
        });

        it("Correct name rendered", () => {
            const name = document.getElementsByClassName("value-main-course-name")[0];
            expect(name).to.not.be.a("null");
            expect(name.innerHTML).to.equal("Breakfast Pizza");
        });

        it("Correct price rendered", () => {
            const price = document.getElementsByClassName("price")[0];
            expect(price).to.not.be.a("null");
            expect(price.innerHTML).to.equal("196 SEK");
        });
